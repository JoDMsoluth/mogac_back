...
/**
 * Tries to read variayble from `process.env` and returns its value.
...
export function getModelFromTypegoose<T extends I.ClassType<Typegoose>>(
  constr: T
) {
  return (new constr().getModelForClass(constr) as unknown) as I.TypegooseModel<
    T
  >;
}
...
>(func: TFn, minDelay: number) {
  const queue = new Coll.Queue<
    [(res: Vts.AsyncReturnType<TFn>) => void, TThis, Parameters<TFn>]
  >();
  let isWaiting = false;
