 ...
 * ```
 */
export type TypegooseDocProps<TInstance extends Typegoose> = FilterProps<
  { TInstance },
  Function
>;
 ...
 *               must not be assignable to.
 */
export type FilterProps<TObj extends Vts.BasicObject, TValue> = Pick<
  TObj,
  FilteredPropNames<TObj, TValue>
>;
 ...
 * ```
 */
export type TypegooseModel<TGDocClass extends ClassType> = TGDocClass &
  Mongoose.Model<TypegooseDocument<TGDocClass>>;

/**
 * Instance type of TypegooseModel<>.
 */
export type TypegooseDocument<TGDocClass extends ClassType> = InstanceType<
  TGDocClass
> &
  Mongoose.Document;
