...
> {
  ...
  async paginate(
    ...
    const mongoSearch =
      search == null
        ? {}
        : _.transform(search, this.transformQueryObj(Paginator.mapSearchValue));

    const mongoExcludeFilter =
      filter == null || filter.exclude == null
        ? null
        : _.transform(
            filter.exclude,
            this.transformQueryObj((value, key) =>
    ...
    const mongoIncludeFilter =
      filter == null || filter.include == null
        ? null
        : _.transform(
            filter.include,
            this.transformQueryObj((value, key) =>
    ...
    const mongoSort = _.transform(
      sort,
      this.transformQueryObj(Paginator.checkSortValue)
    );
